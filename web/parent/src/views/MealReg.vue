      ...
      <el-table :data="tableData" border v-loading="loading">
        ...
        <el-table-column label="在校配餐" width="100">
          <template #default="{ row }">
            <el-tag :type="row.isEat === 1 ? 'success' : 'info'">
              {{ row.isEat === 1 ? '是' : '否' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="就餐人" prop="dinerId">
          <el-select v-model="form.dinerId" placeholder="请选择就餐人" style="width: 100%" @change="onDinerChange">
...
import { getMealRegistrations, createMealRegistration, updateMealRegistration, deleteMealRegistration, getSemesters } from '@/api/parent'
import { getDiners } from '@/api/parent'
...
const semesters = ref([])
const diners = ref([])
const schools = ref([{ id: 1, name: '默认学校' }]) // TODO: 从 API 获取

const queryForm = reactive({ page: 1, size: 10, semesterId: null })
...
const getList = async () => {
  loading.value = true
  try {
    const res = await getMealRegistrations(queryForm)
...
const loadDiners = async () => {
  ...
  diners.value = res.data
}

const onDinerChange = (dinerId) => {
  const diner = diners.value.find(d => d.id === dinerId)
  if (diner) {
    form.idCard = diner.idCard
    form.phone = diner.phone
    form.schoolId = diner.schoolId
  }
}
...
const handleAdd = () => {
  ...
  Object.assign(form, { id: null, semesterId: null, dinerId: null, schoolId: 1, classId: null, dinerName: '', idCard: '', phone: '', isEat: 1, remark: '' })
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该登记吗？', '提示', { type: 'warning' })
    .then(async () => {
      await deleteMealRegistration(row.id)
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (isEdit.value) {
          await updateMealRegistration(form.id, form)
        } else {
          await createMealRegistration(form)
        }
